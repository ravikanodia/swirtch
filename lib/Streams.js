window.Swirtch.Streams = function(response) {
  ...
  that.render = function() {
    ...
    if (response && response.streams.length !== 0) {
      element.appendChild(window.Swirtch.ResultsHeader(response).render());
      element.appendChild(that.renderResultsBody(response));
    } else {
  ...
  that.renderResultsBody = function(response) {
    ...
    console.log('results count: ' + response.streams.length);
    for (var i = 0; i < response.streams.length; i++) {
      results.appendChild(that.renderStream(response.streams[i]));
    }
    return results;
  };
